...
public class EndermiteHatFeature extends FeatureRenderer<EndermiteEntity, EndermiteEntityModel<EndermiteEntity>> {
    private final HeldItemRenderer actuallyItsAHatRenderer;

    public EndermiteHatFeature(FeatureRendererContext<EndermiteEntity, EndermiteEntityModel<EndermiteEntity>> context, HeldItemRenderer handRenderer) {
        super(context);
    ...
    public void render(
        ...
        ItemStack hat = FamiliarAttachments.getHat(endermite);
        if (hat.isEmpty()) {
            return;
        }
        ...
        this.actuallyItsAHatRenderer.renderItem(endermite, hat, ModelTransformationMode.HEAD, false, matrices, vertexConsumers, light);
        matrices.pop();
    }
}
